...
class MPNNLayer(MessagePassing):
    def __init__(self, in_channels_node, out_channels):
        super(MPNNLayer, self).__init__(
            aggr="mean"
        )  # Specify the aggregation method (e.g., mean)
        self.gcn_node = GCNConv(
            in_channels_node, out_channels
        )  # GCN layer for node features
        self.edge_mlp = torch.nn.Linear(
            1, out_channels
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        """
        node_embedding = self.gcn_node(
            x, edge_index
        )  # Apply GCN layer to node features
        edge_embedding = self.edge_mlp(
            edge_attr.view(-1, 1)
        ).relu()  # Apply linear layer to edge features
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        for i, layer in enumerate(self.layers):
            if isinstance(layer, MPNNLayer):
                ...
                if i < len(self.layers) - 1 and isinstance(
                    self.layers[i + 1], LayerNorm
                ):
                    x = x.relu()
...
class MPNNGAE(GAE):
    def recon_loss(self, z, pos_edge_index, num_nodes=None, neg_edge_index=None):
        ...
        """
        pos_loss = -torch.log(
            self.decoder(z, pos_edge_index, num_nodes) + EPS
        ).mean()  # Positive edge loss
        if neg_edge_index is None:
            neg_edge_index = negative_sampling(
                pos_edge_index, num_nodes
            )  # Negative edge sampling
        neg_loss = -torch.log(
            1 - self.decoder(z, neg_edge_index, num_nodes) + EPS
        ).mean()  # Negative edge loss
    ...
    def compute_metrics(self, z, pos_edge_index, neg_edge_index, num_nodes=None):
        ...
        pos_pred = torch.sigmoid(
            self.decoder(z, pos_edge_index, num_nodes)
        )  # Positive predictions
        neg_pred = torch.sigmoid(
            self.decoder(z, neg_edge_index, num_nodes)
...
def train_validate_test(
    ...
    device: str,
    mode: str = "train",
) -> Union[float, Tuple[float, float, float], Tuple[float, float, Tensor]]:
    """
    ...
    for data in tqdm(data_loader):  # Loop over each batch of data in the data loader
        ...
        if (
            mode != "train"
        ):  # If in 'validate' or 'test' mode, compute the ROC AUC score & the AP score
            auc, ap = model.compute_metrics(
                z,
                data.edge_index.to(torch.int64),
                data.negative_edge_index.to(torch.int64),
                data.num_nodes,
            )
    ...
    if mode == "train":  # If in 'train' mode, return average loss
        ...
        mode == "validate"
    ):  # If in 'validate' mode, return average loss, average AUC score, and average AP score
        return (
            float(total_loss / len(data_loader)),
            auc_sum / len(data_loader),
            ap_sum / len(data_loader),
        )
