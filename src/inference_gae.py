...
if __name__ == "__main__": 
    ...
    if config["scope"] == "residue":
        ...
        model.load_state_dict(torch.load(os.path.join(config["model_output_dir"], f"{config["model_name"]}.pt")))
    ...
    for dataset in datasets:
        ...
        df_dataset = pd.DataFrame({"structure_name": latent_spaces.keys(), "structural_embeddings": latent_spaces.values()})
    ...
    df_embeddings.to_csv(os.path.join(config["output_path_embeddings"], f"{config["scope"]}_embeddings.csv"), index=False)    
