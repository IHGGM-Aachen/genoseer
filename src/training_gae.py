...
def parse_arguments():
    ...
    parser.add_argument('--config_path', type=str, help='Path to the model and training config yaml')
...
if __name__ == "__main__": 
    ...
    pdb_paths = [os.path.join(config["input_dir_pdbs"], pdb_path) for pdb_path in os.listdir(config["input_dir_pdbs"]) if pdb_path.endswith(".pdb")]
    ...
    ds_train = constructor.construct_or_load_dataset(f"{config['dataset_name']}_train", training_paths, config["output_path_datasets"], config["num_cores"], config["chunk_size"])
    ...
    ds_valid = constructor.construct_or_load_dataset(f"{config['dataset_name']}_valid", validation_paths, config["output_path_datasets"], config["num_cores"], config["chunk_size"])
    logger.info("Construction of validation data done!")

    # Construct the test dataset
    logger.info("Construction of test data started.")
    ds_test = constructor.construct_or_load_dataset(f"{config['dataset_name']}_test", testing_paths, config["output_path_datasets"], config["num_cores"], config["chunk_size"])
    
    # Initialize the data loaders for training, validation and testing data
    logger.info("Setting up dataloaders.")
    data_loader_train = DataLoader(ds_train, batch_size=config["batch_size"], shuffle=True, drop_last=True)
    data_loader_valid = DataLoader(ds_valid, batch_size=config["batch_size"], shuffle=False, drop_last=True)
    data_loader_test = DataLoader(ds_test, batch_size=config["batch_size"], shuffle=False, drop_last=True)
    ...
    if config["scope"] == "residue":
        num_features = ds_train[0].coords.size(1) + ds_train[0].amino_acid_one_hot.size(1) +  ds_train[0].hbond_acceptors.size(1) + ds_train[0].hbond_donors.size(1)
        model = GCNGAE(GCNEncoder(num_features, out_channels, num_layers), InnerProductDecoder())
        ...
        model = MPNNGAE(MPNNEncoder(num_features, out_channels, num_layers), InnerProductDecoder())
    ...
    for epoch in range(1, epochs+1):
        ...
        train_loss = train_validate_test(data_loader_train, model, optimizer, device, mode='train')
        logger.info(f"Epoch: {epoch}, Train Loss: {train_loss}")  

        # Perform validation and log the validation loss
        valid_loss, valid_auc, valid_ap = train_validate_test(data_loader_valid, model, optimizer, device, mode='validate')
        logger.info(f"Epoch: {epoch}, Validation Loss: {valid_loss}, Validation AUC: {valid_auc}, Validation AP: {valid_ap}")
    ...
    test_auc, test_ap, _ = train_validate_test(data_loader_test, model, optimizer, device, mode='test')
    ...
    torch.save(model.state_dict(), f"{os.path.join(config['model_output_dir'], config['model_name'])}.pt")
